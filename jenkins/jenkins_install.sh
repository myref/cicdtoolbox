...
openssl pkcs12 -export -in jenkins/import.pem -inkey jenkins/jenkins.tooling.provider.test.pem -name jenkins.tooling.provider.test.pem -passout pass:$jenkins_storepass > jenkins/jenkins.p12
...
keytool -import -alias vault.tooling.provider.test -keystore ./jenkins/keystore/cacerts -file ./jenkins/ca.crt -storepass $jenkins_storepass -noprompt
...
if [ -f "jtoken.txt" ]; then
    ...
    if curl -u "jenkins-jenkins:${jtoken}" --insecure 'https://jenkins.tooling.provider.test:8084/crumbIssuer/api/xml?xpath=concat(//crumbRequestField,":",//crumb)'; then
